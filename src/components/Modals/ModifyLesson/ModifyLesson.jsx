...
import styles from "./ModifyLesson.module.scss";
...
import { modifyLessonValidationSchema } from "@/helpers/validation";
import { yupResolver } from "@hookform/resolvers/yup";
import { modifyLesson, getClassLessons } from "@/redux/Teacher/TeacherSlice";
...
const ModifyLesson = () => {
  ...
  const sendRequest = async (data) => {
    ...
    uploadedFile?.file && formData.append("attachment", uploadedFile.file);
    uploadedFile?.thumbnailFile && formData.append("thumbnail", uploadedFile.thumbnailFile);

    let response = await dispatch(modifyLesson(formData));
    if (response.payload.success) {
      dispatch(getClassLessons({class_id}));
      dispatch(showModal({ action: "hide", type: "modifyLesson" }));
    }
  };

  const resolver = yupResolver(modifyLessonValidationSchema);
  ...
  return (
    ...
    <section className={cx(styles.modifyLessonContainer, "flexCol")}>

      <div className={cx(styles.header, "flexRow-space-between")}>
        <Icon onClick={()=>dispatch(showModal({ action: "hide", type: "modifyLesson" }))} icon="carbon:close-filled" color="white" />
      </div>
