...
import UserDetailsModal from "@/components/Modals/AdminDetails/AdminDetails";
...
const AdminUsers = props => {
	...
	const allUsersList = useSelector((state) => state?.user?.getAllAdminUsersData?.data?.data);
	const loading = useSelector((state) => state?.user.loading);
	const modalLoading = useSelector((state) => state?.modalState.loading);
	...
	const [modalStatus, setModalStatus] = useState({
		type: useState(useSelector((state) => state.modalState.type)),
		action: useState(useSelector((state) => state.modalState.action))
	});
	
	useEffect(() => {
		setModalStatus({action: modalStatus.action, type: modalStatus.type});
	},[modalStatus.action, modalStatus.type, modalLoading]);
	...
	const handleViewClick =(id)=>{
		setModalAssetId(id);
		setModalStatus({type: "userDetails", action: "show"});
	};
			...
			<div style={{ width: "100%", minHeight: "50vh", display: "flex", flexDirection: "column", justifyContent: "center"}} className={cx("")}>
				...
				>
					...
					<div style={{ gap: "1rem", marginTop: "2rem" }} className={cx(styles.btnDiv, "flexRow-fully-centered")}>
						...
						<Button loading={loading}  onClick={handleSubmit((data) => modifyUserFxn(data))} title="Modify" textColor="#fff" borderRadiusType="lowRounded" bgColor="#2C0085" />
					</div>
				</form>	
			</div>
		);
	...
	return (
		<>
			{showBtnGroup && <div style={{ justifyContent: "flex-end", gap: "1rem", marginBottom: "1rem"}} className={cx(styles.btnGroup, styles.left,  "flexRow")}>
				<div onClick={() => displayModal("show", "modifyUser")}><Button title="Edit" textColor="#2C0085" borderRadiusType="lowRounded" bordercolor="2C0085" bgColor="#fff" /></div>
			...
			{modalStatus.action === "show" ? <Modal show >{ modalStatus.type === "userDetails" ? viewUserDetails(modalAssetId) : modalStatus.type === "deleteUser" ? confirmDeleteModal() : modalStatus.type === "modifyUser" ? confirmModifyModal() : null}</Modal> : null}
