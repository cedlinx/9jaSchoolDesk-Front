...
const ReportedStolen = props => {

	const dispatch = useDispatch();

	const loading = useSelector((state)=>state?.assets?.loading);
	const allMissingAssets = useSelector((state) => state?.assets?.chartData?.data?.missing_assets);
	...
	const getAssetDetails = (data) => {
		return allMissingAssets?.filter(asset=> asset.id === data)[0];
	};
	...
	const columnsHeader = [
		...
		},
		{
			...
			Cell: ()=>{
				return <div>
					<Icon icon="lucide:alert-triangle" color="#ff9906" />				<span style={{ color: "#ff9906"}}>Stolen</span>	
				</div>;
			}
		},
		{
		...
		<>
			...
			<div className={cx("flexCol")}>
				{loading ? <TableSkeleton /> : 
					<TableComponent columnsHeader={columnsHeader} tableData={getTableData(allMissingAssets)} selectedRowsData={selectedRowsData} />}
			</div>
...
ReportedStolen.propTypes = {
    
};

export default ReportedStolen;